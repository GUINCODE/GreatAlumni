   <?php
    ...
    $passwordRec=$_POST["z_f_psw_user"];
    $z_f_psw_user = password_hash($passwordRec,  PASSWORD_DEFAULT);
    ...
    if($passwordRec !=""){
        ...
        $stmt = $db->prepare("UPDATE `utilisateur` SET `Nom` = :z_f_nom_user,`Prenom` = :z_f_prenom_user, `Mail`=:z_f_mail_user,  `Loginn` = :z_f_login_user,  `Typee`=:z_f_role_user  WHERE `id`=:z_f_id_user ");
        $stmt->bindParam(':z_f_id_user', $z_f_id_user);
        $stmt->bindParam(':z_f_nom_user', $z_f_nom_user);
        $stmt->bindParam(':z_f_prenom_user', $z_f_prenom_user);
        $stmt->bindParam(':z_f_mail_user', $z_f_mail_user);
